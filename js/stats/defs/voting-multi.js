...
// Formula: (1 + meritoc9/100) * (1 + innerSum/100)
...
export default {
  ...
  combine: function(pools, ctx) {
    ...
    var meritoc9 = computeMeritocBonusz(9);
    ...
    var legendPTS22 = legendPTSbonus(22);
    ...
    var winBonus22 = computeWinBonus(22);
    ...
    var val = (1 + meritoc9 / 100) * (1 + innerSum / 100);
    ...
    if (paletteBonus32 > 0) innerCh.push({ name: label('Palette', 32), val: paletteBonus32,
      children: [
        { name: 'Palette Lv', val: paletteLv, fmt: 'raw' },
        { name: label('Legend', 10, ' \u00d7'), val: 1 + legendPTSbonus(10) / 100, fmt: 'x' },
      ], fmt: 'raw' });
    if (legendPTS22 > 0) innerCh.push({ name: label('Legend', 22), val: legendPTS22, fmt: 'raw' });

    var children = [
      { name: label('Meritoc', 9, ' \u00d7'), val: 1 + meritoc9 / 100, fmt: 'x' },
      { name: 'Inner \u00d7', val: 1 + innerSum / 100, children: innerCh.length ? innerCh : null, fmt: 'x' },
    ];
