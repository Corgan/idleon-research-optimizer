...
export function resourceAllocationToReset(S, options) {
	...
	const details = allocation.details.map(detail => {
		...
		return { ...detail, collectableBeforeReset: detail.collectedWithinWindow, emptiesByReset, initiallyDrained, initiallyEmpty, refillsAtReset, refillAtReset: refillsAtReset, refillBoundary: initiallyDrained ? 'source-certain' : emptiesByReset ? 'projected-uncertain' : 'none', gradeGainAtReset, projectedGradeAtReset: R.resourceGrade(S, detail.resourceIdx) + gradeGainAtReset };
	});
	return { ...allocation, available: true, resetProjectionAvailable: true, reset, resetLabel: 'before reset', hoursToReset: reset.hoursRemaining, collectableBeforeReset: allocation.collectedWithinWindow, nodesEmptyingByReset: allocation.nodesEmptiedWithinWindow, projectedRefillsAtReset: details.filter(detail => detail.refillsAtReset).length, projectedGradeGainsAtReset: details.reduce((sum, detail) => sum + detail.gradeGainAtReset, 0), details, projection: 'Reset-boundary projection; projected-to-empty refill outcomes are uncertain and post-reset state is not simulated' };
}
...
export function currencyIncomeToReset(S, currencySlot, options) {
	...
	const nodes = breakdown.nodes.map(node => { const detail = resetDetails.get(node.resourceIdx); return projectionAvailable ? { ...node, collectableBeforeReset: node.collectableWithinWindow, emptiesByReset: detail?.emptiesByReset || false, initiallyDrained: detail?.initiallyDrained || false, initiallyEmpty: detail?.initiallyEmpty || false, refillsAtReset: detail?.refillsAtReset || false, refillAtReset: detail?.refillAtReset || false, refillBoundary: detail?.refillBoundary || 'none', gradeGainAtReset: detail?.gradeGainAtReset || 0, projectedGradeAtReset: detail?.projectedGradeAtReset ?? node.grade, resetProjectionAvailable: true } : { ...node, collectableBeforeReset: null, emptiesByReset: null, initiallyDrained: node.drained, initiallyEmpty: node.alreadyEmpty, refillsAtReset: false, refillAtReset: false, refillBoundary: null, gradeGainAtReset: null, projectedGradeAtReset: null, resetProjectionAvailable: false }; });
	return projectionAvailable ? { ...breakdown, resetProjectionAvailable: true, reset, resetLabel: 'before reset', hoursToReset: reset.hoursRemaining, collectableBeforeReset: nodes.reduce((sum, node) => sum + node.collectableBeforeReset, 0), nodes, projectedRefillsAtReset: nodes.filter(node => node.refillsAtReset).length, projectedGradeGainsAtReset: nodes.reduce((sum, node) => sum + node.gradeGainAtReset, 0), projection: 'Reset-boundary projection; projected-to-empty refill outcomes are uncertain and post-reset state is not simulated' } : { ...breakdown, resetProjectionAvailable: false, partial: true, reset, resetLabel: 'before reset', hoursToReset: null, collectableBeforeReset: null, nodes, projectedRefillsAtReset: null, projectedGradeGainsAtReset: null, missing: [...new Set([...(breakdown.missing || []), ...reset.missing])], projection: 'Reset-boundary projection unavailable; current node state preserved without reset claims' };
}
