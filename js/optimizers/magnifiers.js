...
export async function optimizeMagsFor(s, ctx) {
  ...
  let assigned = await _enumKalMags(availSlots, numKalei, regsToPlace, kalBase, gd101, il, maxPerSlot);
...
export function _evalMagScoreWith(trialMags, gl, so, il, occ, rLv) {
  const ctx = makeCtx(gl);
  const kalMap = _buildKalMap(trialMags);
  const kalBase = getKaleiMultiBase(gl, so, ctx);
  const gd101 = _gbWith(gl, so, 93, ctx);
