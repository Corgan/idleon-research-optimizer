...
import { _gbWith, deathNoteRank } from '../sim-math.js';
import {
  emporiumBonus,
  eventShopOwned,
  ribbonBonusAt,
...
export function getOverrideOrAuto(key, autoVal) {
...
export function computeMeasurementMulti(typeIdx) {
...
export function computeGambitPTSmulti() {
...
export function computeExternalBonuses() {
  ...
  // Compute allBonusMulti early so getGridBonusFinal calls below use the correct value
  const _comp55val = companionIds.has(55) ? 15 : 0;
  const _comp0val = companionIds.has(0) && cachedComp0DivOk && (gridLevels[173] || 0) > 0 ? 5 : 0;
...
export function computeGambitBonus15() {
