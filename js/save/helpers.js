...
export function eventShopOwned(t) {
  ...
  return S.cachedEventShopStr.includes(N2L[t]) ? 1 : 0;
}

export function superBitType(t) {
  if (t < 0 || t >= N2L.length) return 0;
  return String(S.gamingData[12] || '').includes(N2L[t]) ? 1 : 0;
}

export function emporiumBonus(t) {
  if (t < 0 || t >= N2L.length) return 0;
  return String(S.ninjaData?.[102]?.[9] || '').includes(N2L[t]) ? 1 : 0;
}

export function ribbonBonusAt(index) {
  const t = S.ribbonData[index] || 0;
  if (t <= 0) return 1;
  const hasEmperorSet = String(S.olaData[379] || '').includes('EMPEROR_SET');
